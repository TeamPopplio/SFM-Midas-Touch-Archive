...
{
	IClientEntity* portal = getPortal(_y_midas_overlay_portal.GetString(), false);
...
void calculateOffsetPlayer(IClientEntity* saveglitch_portal, Vector& new_player_origin, QAngle& new_player_angles)
{
	...
	else
	{
		...
		static int offset = midas_entutils.GetFieldOffset("CProp_Portal", "m_matrixThisToLinked", true);
		if (e && offset != -1)
		{
