...
{
	return _y_midas_overlay.GetBool();
}

bool OverlayRenderer::shouldFlipScreens()
{
	return _y_midas_overlay_swap.GetBool();
}
...
void OverlayRenderer::modifyView(CViewSetup* view, bool overlay)
{
	...
	else
	{
		...
		switch (_y_midas_overlay_type.GetInt())
		{
		case 0:
			data = sgOverlay();
			break;
		...
		if (!shouldFlipScreens())
		{
			int width = _y_midas_overlay_width.GetFloat();
			int height = static_cast<int>(width / midas_overlay.GetScreenAspectRatio());
			view->width = width;
			view->height = height;
		}

		view->fov = getFOV();
		view->m_flAspectRatio = midas_overlay.GetScreenAspectRatio();
	}
}
...
{
	const float ratioRatio = midas_overlay.GetScreenAspectRatio() / (4.0f / 3.0f);
	float fovRad = DEG2RAD(_y_midas_overlay_fov.GetFloat());
