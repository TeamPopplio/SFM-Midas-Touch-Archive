...
ConVar y_midas_ihud("y_midas_ihud", "0", 0, "Draws movement inputs of client.\n");
ConVar y_midas_ihud_grid_size("y_midas_ihud_grid_size", "60", 0, "Grid size of input HUD.\n");
ConVar y_midas_ihud_grid_padding("y_midas_ihud_grid_padding", "2", 0, "Padding between grids of input HUD.");
ConVar y_midas_ihud_x("y_midas_ihud_x", "2", 0, "X offset of input HUD.\n");
ConVar y_midas_ihud_y("y_midas_ihud_y", "2", 0, "Y offset of input HUD.\n");

CON_COMMAND(
    y_midas_ihud_modify,
    "y_midas_ihud_modify <element|all> <param> <value> - Modifies parameters in given element.\nParams: enabled, text, font, x, y, width, height, background, highlight, textcolor, texthighlight.\n")
{
	if (args.ArgC() != 4)
	{
		Msg("Usage: y_midas_ihud_modify <element|all> <param> <value>\n");
		return;
	}
	const char* errorMsg =
	...
	{
		for (const auto& kv : midas_ihud.buttonSetting)
		{
			if (!midas_ihud.ModifySetting(kv.first.c_str(), param, value))
			{
				Msg(errorMsg);
				return;
			}
		...
		{
			midas_ihud.ModifySetting("angles", param, value);
		}
	}
	else if (!midas_ihud.ModifySetting(element, param, value))
	{
		Msg(errorMsg);
	}
}

CON_COMMAND(y_midas_ihud_preset, "Load ihud preset.\nValid options: normal, normal_mouse, tas.\n")
{
	if (args.ArgC() != 2)
	{
		Msg("Usage: y_midas_ihud_preset <preset>\nPresets: normal, normal_mouse, tas.\n");
	}
	const char* preset = args.Arg(1);
	if (std::strcmp(preset, "normal") == 0)
	{
		midas_ihud.tasPreset = false;
		InputHud::Button* button = &midas_ihud.buttonSetting["forward"];
		button->enabled = true;
		button->x = 0;
		button->y = 2;
		button->width = 1;
		button->hight = 1;
		button = &midas_ihud.buttonSetting["use"];
		button->enabled = true;
		button->x = 0;
		button->y = 3;
		button->width = 1;
		button->hight = 1;
		button = &midas_ihud.buttonSetting["moveleft"];
		button->enabled = true;
		button->x = 1;
		button->y = 1;
		button->width = 1;
		button->hight = 1;
		button = &midas_ihud.buttonSetting["back"];
		button->enabled = true;
		button->x = 1;
		button->y = 2;
		button->width = 1;
		button->hight = 1;
		button = &midas_ihud.buttonSetting["moveright"];
		button->enabled = true;
		button->x = 1;
		button->y = 3;
		button->width = 1;
		button->hight = 1;
		button = &midas_ihud.buttonSetting["duck"];
		button->enabled = true;
		button->x = 2;
		button->y = 0;
		button->width = 1;
		button->hight = 1;
		button = &midas_ihud.buttonSetting["jump"];
		button->enabled = true;
		button->x = 2;
		button->y = 1;
		button->width = 3;
		button->hight = 1;
		button = &midas_ihud.buttonSetting["attack"];
		button->enabled = true;
		button->x = 2;
		button->y = 4;
		button->width = 1;
		button->hight = 1;
		button = &midas_ihud.buttonSetting["attack2"];
		button->enabled = true;
		button->x = 2;
		button->y = 5;
		button->width = 1;
		...
		midas_ihud.anglesSetting.enabled = false;
	}
	else if (std::strcmp(preset, "normal_mouse") == 0)
	{
		...
		midas_ihud.anglesSetting.enabled = true;
		midas_ihud.anglesSetting.x = 1;
		midas_ihud.anglesSetting.y = 4;
	}
	else if (std::strcmp(preset, "tas") == 0)
	{
		midas_ihud.tasPreset = true;
	}
	else
	{
		Msg("Invalid preset.\nPresets: normal, normal_mouse, tas.\n");
...
{
	return midas_hud.ShouldLoadFeature();
}
...
void InputHud::LoadFeature()
{
	...
	bool result = AddHudCallback("ihud", std::bind(&InputHud::DrawInputHud, this), y_midas_ihud);
	if (result)
	{
		InitCommand(y_midas_ihud_modify);
		InitCommand(y_midas_ihud_preset);
		InitConcommandBase(y_midas_ihud_grid_size);
		InitConcommandBase(y_midas_ihud_grid_padding);
		InitConcommandBase(y_midas_ihud_x);
		InitConcommandBase(y_midas_ihud_y);
	}
	vgui::HFont font = midas_hud.scheme->GetFont("Trebuchet20", false);
...
bool InputHud::ModifySetting(const char* element, const char* param, const char* value)
{
	...
	{
		target->font = midas_hud.scheme->GetFont(value, false);
	}
...
void InputHud::DrawInputHud()
{
	...
	surface = midas_hud.surface;

	xOffset = y_midas_ihud_x.GetInt();
	yOffset = y_midas_ihud_y.GetInt();
	gridSize = y_midas_ihud_grid_size.GetInt();
	padding = y_midas_ihud_grid_padding.GetInt();

	if (y_midas_ihud.GetBool())
	{
		Vector ang;
...
{
	midas_ihud.ORIG_DecodeUserCmdFromBuffer(thisptr, edx, buf, sequence_number);

	auto m_pCommands =
	    *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(thisptr) + midas_playerio.offM_pCommands);
	auto pCmd = m_pCommands + midas_playerio.sizeofCUserCmd * (sequence_number % 90);
	auto cmd = reinterpret_cast<CUserCmd*>(pCmd);
	midas_ihud.SetInputInfo(cmd->buttons, Vector(cmd->sidemove, cmd->forwardmove, cmd->upmove));
	pCmd = 0;
}
