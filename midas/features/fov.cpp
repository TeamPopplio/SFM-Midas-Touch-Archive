...
ConVar _y_midas_force_fov("_y_midas_force_fov", "0", 0, "Force FOV to some value.");
...
void FOVFeatures::PreHook()
{
	if (_viewmodel_fov && ORIG_CViewRender__OnRenderStart)
		InitConcommandBase(_y_midas_force_fov);
}

void __fastcall FOVFeatures::HOOKED_CViewRender__OnRenderStart(void* thisptr, int edx)
{
	midas_fov.ORIG_CViewRender__OnRenderStart(thisptr, edx);

	if (!_viewmodel_fov || !_y_midas_force_fov.GetBool())
		return;
	...
	*fov = _y_midas_force_fov.GetFloat();
