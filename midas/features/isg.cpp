...
ConVar y_midas_hud_isg("y_midas_hud_isg", "0", FCVAR_CHEAT, "Is the ISG flag set?\n");
...
protected:
	...
	virtual void PreHook() override
	{
		if (ORIG_MiddleOfRecheck_ov_element)
			...
			Warning("y_midas_hud_isg 1 and y_midas_set_isg have no effect\n");
	}
...
static ISGFeature midas_isg;
CON_COMMAND_F(y_midas_set_isg,
              "Sets the state of ISG in the game (1 or 0), no arguments means 1",
              FCVAR_DONTRECORD | FCVAR_CHEAT)
{
	if (midas_isg.isgFlagPtr)
		*midas_isg.isgFlagPtr = args.ArgC() == 1 || atoi(args[1]);
	else
		Warning("y_midas_set_isg has no effect\n");
}

bool IsISGActive()
{
	if (midas_isg.isgFlagPtr)
		return *midas_isg.isgFlagPtr;
	else
		return false;
}
...
void ISGFeature::LoadFeature()
{
	if (ORIG_MiddleOfRecheck_ov_element)
	{
		InitCommand(y_midas_set_isg);

#ifdef SSDK2007
		AddHudCallback(
		    "isg", []() { midas_hud.DrawTopHudElement(L"isg: %d", IsISGActive()); }, y_midas_hud_isg);
#endif
	}
}
