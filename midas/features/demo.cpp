...
DemoStuff midas_demostuff;

ConVar y_midas_pause_demo_on_tick(
    "y_midas_pause_demo_on_tick",
    "0",
    0,
...
void DemoStuff::PreHook()
{
	...
	if (ORIG_Record)
	{
		...
		else if (index == 1)
		{
			...
			Warning(
			    "Record pattern had no matching clause for catching the demoplayer. y_midas_pause_demo_on_tick unavailable.\n");
...
void DemoStuff::LoadFeature()
{
	...
	{
		InitConcommandBase(y_midas_pause_demo_on_tick);
...
{ // This hook will get called twice per loaded save (in most games/versions, at least, according to SAR people), once with m_bLoadgame being false and the next one being true
	...
	{
		midas_demostuff.ORIG_StopRecording(thisptr, edx);
		midas_demostuff.isAutoRecordingDemo = false;
		midas_demostuff.currentAutoRecordDemoNumber = 1;
		return;
	}

	bool* pM_bRecording = (bool*)((uint32_t)thisptr + midas_demostuff.m_bRecording_Offset);
	int* pM_nDemoNumber = (int*)((uint32_t)thisptr + midas_demostuff.m_nDemoNumber_Offset);

	// This will set m_nDemoNumber to 0 and m_bRecording to false
	midas_demostuff.ORIG_StopRecording(thisptr, edx);

	if (midas_demostuff.isAutoRecordingDemo)
	{
		*pM_nDemoNumber = midas_demostuff.currentAutoRecordDemoNumber;
		*pM_bRecording = true;
	}
	else
	{
...
void __fastcall DemoStuff::HOOKED_SetSignonState(void* thisptr, int edx, int state)
{
	...
	if (midas_demostuff.ORIG_StopRecording && scripts::g_TASReader.IsExecutingScript())
	{
		bool* pM_bRecording = (bool*)((uint32_t)thisptr + midas_demostuff.m_bRecording_Offset);
		int* pM_nDemoNumber = (int*)((uint32_t)thisptr + midas_demostuff.m_nDemoNumber_Offset);
		...
		if (state == 5 && midas_demostuff.isAutoRecordingDemo)
		{
			midas_demostuff.currentAutoRecordDemoNumber++;
		}
		...
		else if (state == 6)
		{
			...
			if (*pM_bRecording)
			{
				midas_demostuff.isAutoRecordingDemo = true;
				*pM_nDemoNumber = midas_demostuff.currentAutoRecordDemoNumber;
			}
		}
	}
	midas_demostuff.ORIG_SetSignonState(thisptr, edx, state);
}

void __cdecl DemoStuff::HOOKED_Stop()
{
	midas_demostuff.isAutoRecordingDemo = false;
	if (midas_demostuff.ORIG_Stop)
		midas_demostuff.ORIG_Stop();
}
...
void DemoStuff::OnTick()
{
	...
	{
		auto tick = y_midas_pause_demo_on_tick.GetInt();
		if (tick != 0)
		{
			if (tick < 0)
