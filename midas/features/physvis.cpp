...
int __fastcall PhysVisFeature::HOOKED_CPhysicsCollision__CreateDebugMesh(const IPhysicsCollision* thisptr,
                                                                         ...
                                                                         Vector** outVerts)
{
	auto& state = midas_physvis.renderState;
	int numVerts;
	if (state.verts)
	{
	...
	{
		numVerts = midas_physvis.ORIG_CPhysicsCollision__CreateDebugMesh(thisptr, dummy, pCollideModel, outVerts);
	}
...
void __fastcall PhysVisFeature::HOOKED_CPhysicsCollision__DestroyDebugMesh(const IPhysicsCollision* thisptr,
                                                                           ...
                                                                           Vector* outVerts)
{
	if (!midas_physvis.renderState.verts)
...
void __fastcall PhysVisFeature::HOOKED_CRendering3dView__DrawTranslucentRenderables(void* thisptr,
                                                                                    ...
                                                                                    bool bShadowDepth)
{
	midas_physvis.ORIG_CRendering3dView__DrawTranslucentRenderables(thisptr, edx, bInSkybox, bShadowDepth);
	// this seems to be the best spot in the pipeline to render meshes, otherwise stuff gets drawn on top of them
	midas_physvis.renderState.renderingCallbacks = true;
	for (auto& callback : midas_physvis.callbacks)
		callback.render();
	midas_physvis.renderState.renderingCallbacks = false;
}
...
void PhysVisFeature::DrawPhysCollideWrapper(const CPhysCollide* pCollide,
                                            ...
                                            const matrix3x4_t* m)
{
	midas_physvis.renderState.curMat = m;
	midas_physvis.renderState.curDrawInfo = &info;
	...
	case FacesAndWire:
	{
		...
		midas_physvis.ORIG_DebugDrawPhysCollide(pCollide, material, actualMat, color, false);
		if (info.wireframeMode == OnlyFaces)
			break;
		color = {0, 0, 0, 250}; // fall through and use a black wireframe
	}
	case OnlyWire:
		midas_physvis.ORIG_DebugDrawPhysCollide(pCollide, nullptr, actualMat, color, false);
	}
	midas_physvis.renderState.curDrawInfo = nullptr;
	midas_physvis.renderState.curMat = nullptr;
}

void PhysVisFeature::DrawCPhysCollide(const CPhysCollide* pCollide, const PhysVisDrawInfo& info, const matrix3x4_t* mat)
{
	if (!midas_physvis.renderState.renderingCallbacks || !pCollide)
		return;
...
{
	if (!midas_physvis.renderState.renderingCallbacks || !pPhysicsObject)
		return;
	Vector pos;
	QAngle ang;
	matrix3x4_t mat;
	midas_physvis.ORIG_CPhysicsObject__GetPosition(pPhysicsObject, 0, &pos, &ang);
...
{
	if (!midas_physvis.renderState.renderingCallbacks || !pEnt)
		return;
#if defined(SSDK2007)
...
{
	if (!midas_physvis.renderState.renderingCallbacks || !verts)
		return;
	auto& state = midas_physvis.renderState;
	state.verts = verts;
