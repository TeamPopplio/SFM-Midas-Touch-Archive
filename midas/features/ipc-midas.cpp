...
namespace ipc
{
...
#endif
	ConVar y_midas_ipc("y_midas_ipc", "0", 0, "Enables IPC.", IPC_Changed);
	ConVar y_midas_ipc_port("y_midas_ipc_port", "27182", 0, "Port used for IPC.");
	...
	void StartIPC()
	{
		...
		{
			ipc::InitWinsock();
			server.StartListening(y_midas_ipc_port.GetString());
		}
	}
...
#if !defined(OE)
CON_COMMAND(y_midas_ipc_ent, "y_midas_ipc_ent <entity index> - Outputs entity data to IPC client.\n")
{
	if (args.ArgC() < 2)
	{
		Msg("Usage: y_midas_ipc_ent <entity index>\n");
		return;
	}
...
#if !defined(OE)
CON_COMMAND(
    y_midas_ipc_properties,
    "y_midas_ipc_properties <entity index> <property1> <property2> ... - Outputs entity properties to IPC client.\n")
{
	if (args.ArgC() < 2)
	{
		Msg("Usage: y_midas_ipc_properties <entity index> <property1> <property2> ...\n");
		return;
	}
	else if (!ipc::IsActive())
	{
...
CON_COMMAND(y_midas_ipc_echo,
            "y_midas_ipc_echo <text> - Sends a text message to IPC client. Don't insert fancy characters into this.\n")
{
	if (args.ArgC() < 2)
	{
		Msg("Usage: y_midas_ipc_echo <text>\n");
		return;
	}
	else if (!ipc::IsActive())
	{
...
CON_COMMAND(y_midas_ipc_playback, "Outputs the current playback tick to IPC client.\n")
{
	if (!ipc::IsActive())
	{
		Msg("No IPC client connected.\n");
...
CON_COMMAND(y_midas_ipc_gamedir, "Output the game directory path to IPC client.\n")
{
	if (!ipc::IsActive())
	{
		Msg("No IPC client connected.\n");
...
#ifndef OE
		InitCommand(y_midas_ipc_ent);
		InitCommand(y_midas_ipc_properties);
#endif
		InitCommand(y_midas_ipc_echo);
		InitCommand(y_midas_ipc_playback);
		InitCommand(y_midas_ipc_gamedir);

		InitConcommandBase(y_midas_ipc);
		InitConcommandBase(y_midas_ipc_port);
	}
}