...
PlayerIOFeature midas_playerio;
static void* cinput_thisptr = nullptr;

ConVar y_midas_hud_accel("y_midas_hud_accel", "0", FCVAR_CHEAT, "Turns on the acceleration hud.\n");
ConVar y_midas_hud_ag_sg_tester("y_midas_hud_ag_sg_tester",
                              "0",
                              FCVAR_CHEAT,
                              "Tests if angle glitch will save glitch you.\n");
ConVar y_midas_hud_flags("y_midas_hud_flags", "0", FCVAR_CHEAT, "Turns on the flags hud.\n");
ConVar y_midas_hud_moveflags("y_midas_hud_moveflags", "0", FCVAR_CHEAT, "Turns on the move type hud.\n");
ConVar y_midas_hud_movecollideflags("y_midas_hud_movecollideflags", "0", FCVAR_CHEAT, "Turns on the move collide hud.\n");
ConVar y_midas_hud_collisionflags("y_midas_hud_collisionflags", "0", FCVAR_CHEAT, "Turns on the collision group hud.\n");
ConVar y_midas_hud_vars("y_midas_hud_vars", "0", FCVAR_CHEAT, "Turns on the movement vars HUD.\n");
ConVar y_midas_hud_velocity("y_midas_hud_velocity", "0", FCVAR_CHEAT, "Turns on the velocity hud.\n");
ConVar y_midas_hud_velocity_angles("y_midas_hud_velocity_angles", "0", FCVAR_CHEAT, "Display velocity Euler angles.");

bool PlayerIOFeature::ShouldLoadFeature()
{
	return interfaces::engine != nullptr && midas_entutils.ShouldLoadFeature();
}
...
void __fastcall PlayerIOFeature::HOOKED_CreateMove(void* thisptr,
                                                   ...
                                                   bool active)
{
	midas_playerio.HOOKED_CreateMove_Func(thisptr, edx, sequence_number, input_sample_frametime, active);
}
...
{
	return midas_playerio.HOOKED_GetButtonBits_Func(thisptr, edx, bResetState);
}
...
void PlayerIOFeature::GetPlayerFields()
{
	...
	if (midas_entutils.ShouldLoadFeature())
	{
		m_afPhysicsFlags = midas_entutils.GetPlayerField<int>("m_afPhysicsFlags");
		m_bDucking = midas_entutils.GetPlayerField<bool>("m_Local.m_bDucking");
		m_CollisionGroup = midas_entutils.GetPlayerField<int>("m_CollisionGroup");
		m_fFlags = midas_entutils.GetPlayerField<int>("m_fFlags");
		m_flDuckJumpTime = midas_entutils.GetPlayerField<float>("m_Local.m_flDuckJumpTime");
		m_flMaxspeed = midas_entutils.GetPlayerField<float>("m_flMaxspeed");
		m_hGroundEntity = midas_entutils.GetPlayerField<int>("m_hGroundEntity");
		m_MoveCollide = midas_entutils.GetPlayerField<int>("m_MoveCollide");
		m_MoveType = midas_entutils.GetPlayerField<int>("m_MoveType");
		m_vecAbsOrigin = midas_entutils.GetPlayerField<Vector>("m_vecAbsOrigin");
		m_vecAbsVelocity = midas_entutils.GetPlayerField<Vector>("m_vecAbsVelocity");
		m_vecPreviouslyPredictedOrigin = midas_entutils.GetPlayerField<Vector>("m_vecPreviouslyPredictedOrigin");
		m_vecPunchAngle = midas_entutils.GetPlayerField<QAngle>("m_Local.m_vecPunchAngle");
		m_vecPunchAngleVel = midas_entutils.GetPlayerField<QAngle>("m_Local.m_vecPunchAngleVel");
		m_vecViewOffset = midas_entutils.GetPlayerField<Vector>("m_vecViewOffset");
		offServerAbsOrigin = m_vecAbsOrigin.field.serverOffset;

		int m_bSinglePlayerGameEndingOffset = midas_entutils.GetPlayerOffset("m_bSinglePlayerGameEnding", true);
...
bool PlayerIOFeature::IsGroundEntitySet()
{
	if (tas_strafe_version.GetInt() <= 4)
	{
		...
		auto player = midas_entutils.GetPlayer(false);
...
#endif
{
	midas_playerio.EnableDuckspam();
}
static ConCommand DuckspamDown_Command("+y_midas_duckspam", DuckspamDown, "Enables the duckspam.");
...
#endif
{
	midas_playerio.DisableDuckspam();
}
static ConCommand DuckspamUp_Command("-y_midas_duckspam", DuckspamUp, "Disables the duckspam.");

CON_COMMAND(_y_midas_getvel, "Gets the last velocity of the player.")
{
	const Vector vel = midas_playerio.GetPlayerVelocity();
...
CON_COMMAND(y_midas_find_portals, "Prints info for all portals")
{
	for (int i = 0; i < MAX_EDICTS; ++i)
	{
		...
		if (!invalidPortal(ent))
		{
			...
			Msg("Midas: There's %s %s portal with index %d at %.8f %.8f %.8f.\n",
			    openStr,
			    color,
			    i,
			    origin.x,
...
void calculate_offset_player_pos(edict_t* saveglitch_portal, Vector& new_player_origin, QAngle& new_player_angles)
{
	...
	auto& player_origin = *reinterpret_cast<Vector*>(reinterpret_cast<uintptr_t>(utils::GetServerPlayer())
	                                                 + midas_playerio.offServerAbsOrigin);
...
CON_COMMAND(
    y_midas_calc_relative_position,
    "y_midas_calc_relative_position <index of the save glitch portal | \"blue\" | \"orange\"> [1 if you want to teleport there instead of just printing]")
{
	if (args.ArgC() != 2 && args.ArgC() != 3)
	{
		Msg("Usage: y_midas_calc_relative_position <index of the save glitch portal | \"blue\" | \"orange\"> [1 if you want to teleport there instead of just printing]\n");
		return;
	}
...
CON_COMMAND(_y_midas_getangles, "Gets the view angles of the player.")
{
...
void DrawFlagsHud(bool mutuallyExclusiveFlags, const wchar* hudName, const wchar** nameArray, int count, int flags)
{
	for (int u = 0; u < count; ++u)
	{
		if (nameArray[u])
		{
			...
			{
				midas_hud.DrawTopHudElement(L"%s: %s", hudName, nameArray[u]);
			}
			else if (!mutuallyExclusiveFlags)
			{
				midas_hud.DrawTopHudElement(L"%s: %d", nameArray[u], (flags & (1 << u)) != 0);
			}
		}
	}
}
...
void PlayerIOFeature::LoadFeature()
{
	...
	if (interfaces::engine)
	{
		InitCommand(_y_midas_getangles);
	}
	...
	if (playerioAddressesWereFound)
	{
		...
		AddHudCallback(
		    ...
		    [this]() {
			    ...
			    midas_hud.DrawTopHudElement(L"accelerate: %.3f", vars.Accelerate);
			    midas_hud.DrawTopHudElement(L"airaccelerate: %.3f", vars.Airaccelerate);
			    midas_hud.DrawTopHudElement(L"ent friction: %.3f", vars.EntFriction);
			    midas_hud.DrawTopHudElement(L"frametime: %.3f", vars.Frametime);
			    midas_hud.DrawTopHudElement(L"friction %.3f", vars.Friction);
			    midas_hud.DrawTopHudElement(L"maxspeed: %.3f", vars.Maxspeed);
			    midas_hud.DrawTopHudElement(L"stopspeed: %.3f", vars.Stopspeed);
			    midas_hud.DrawTopHudElement(L"wishspeed cap: %.3f", vars.WishspeedCap);
			    midas_hud.DrawTopHudElement(L"onground: %d", (int)vars.OnGround);
		    },
		    y_midas_hud_vars);
#endif
	}
	...
	if (m_vecAbsVelocity.Found())
	{
		...
		if (TickSignal.Works)
		{
			...
			AddHudCallback(
			    ...
			    [this]() {
				    ...
				    midas_hud.DrawTopHudElement(L"accel(xyz): %.3f %.3f %.3f", accel.x, accel.y, accel.z);
				    midas_hud.DrawTopHudElement(L"accel(xy): %.3f", accel.Length2D());
			    },
			    y_midas_hud_accel);
		}
		...
		AddHudCallback(
		    ...
		    [this]() {
			    ...
			    midas_hud.DrawTopHudElement(L"vel(xyz): %.3f %.3f %.3f",
			                              currentVel.x,
			                              currentVel.y,
			                              currentVel.z);
			    midas_hud.DrawTopHudElement(L"vel(xy): %.3f", currentVel.Length2D());
		    },
		    y_midas_hud_velocity);
		...
		AddHudCallback(
		    ...
		    [this]() {
			    ...
			    midas_hud.DrawTopHudElement(L"vel(p/y/r): %.3f %.3f %.3f", angles.x, angles.y, angles.z);
		    },
		    y_midas_hud_velocity_angles);
		...
		if (utils::DoesGameLookLikePortal())
		{
			AddHudCallback(
			    "ag sg",
			    [this]() {
				    Vector v = midas_playerio.GetPlayerEyePos();
				    QAngle q;

				    std::wstring result = calculateWillAGSG(v, q);
				    midas_hud.DrawTopHudElement(L"ag sg: %s", result.c_str());
			    },
			    y_midas_hud_ag_sg_tester);
		}
#endif
	}
	...
	{
		InitCommand(y_midas_calc_relative_position);
	}
#endif
#if defined(SSDK2007)
	...
	{
		InitCommand(y_midas_find_portals);
	}
	...
	if (m_fFlags.Found())
	{
		AddHudCallback(
		    "fl_",
		    [this]() {
			    int flags = midas_playerio.m_fFlags.GetValue();
	...
	if (m_CollisionGroup.Found())
	{
		AddHudCallback(
		    "collisionflags",
		    [this]() {
			    int flags = midas_playerio.m_CollisionGroup.GetValue();
			    DrawFlagsHud(true,
			                 L"Collision group",
			                 COLLISION_GROUPS,
