...
ConVar y_midas_hud_hops("y_midas_hud_hops", "0", FCVAR_CHEAT, "When set to 1, displays the hop practice HUD.");
ConVar y_midas_hud_hops_x("y_midas_hud_hops_x", "-85", FCVAR_CHEAT, "Hops HUD x offset");
ConVar y_midas_hud_hops_y("y_midas_hud_hops_y", "100", FCVAR_CHEAT, "Hops HUD y offset");
...
void HopsHud::LoadFeature()
{
	...
	bool result = AddHudCallback("hops", std::bind(&HopsHud::DrawHopHud, this), y_midas_hud_hops);

	if (result)
	{
		InitConcommandBase(y_midas_hud_hops_x);
		InitConcommandBase(y_midas_hud_hops_y);
	}
}
...
void HopsHud::DrawHopHud()
{
	if (hopsFont == 0)
	{
		if (!midas_hud.scheme)
			return;
		hopsFont = midas_hud.scheme->GetFont("Trebuchet24", false);
	}

	auto surface = midas_hud.surface;
	auto screen = midas_hud.screen;
	...
	surface->DrawSetTextPos(screen->width / 2 + y_midas_hud_hops_x.GetFloat(),
	                        screen->height / 2 + y_midas_hud_hops_y.GetFloat());
	surface->DrawPrintText(buffer, wcslen(buffer));

	swprintf_s(buffer, BUFFER_SIZE, L"Speed loss: %.3f", loss);
	surface->DrawSetTextPos(screen->width / 2 + y_midas_hud_hops_x.GetFloat(),
	                        screen->height / 2 + y_midas_hud_hops_y.GetFloat() + (fontTall + MARGIN));
	surface->DrawPrintText(buffer, wcslen(buffer));

	swprintf_s(buffer, BUFFER_SIZE, L"Percentage: %.3f", percentage);
	surface->DrawSetTextPos(screen->width / 2 + y_midas_hud_hops_x.GetFloat(),
	                        screen->height / 2 + y_midas_hud_hops_y.GetFloat() + (fontTall + MARGIN) * 2);
	surface->DrawPrintText(buffer, wcslen(buffer));
}

void HopsHud::Jump()
{
	if (!y_midas_hud_hops.GetBool())
		return;
...
void HopsHud::OnGround(bool onground)
{
	...
	if (!onground)
	{
		...
		if (velNotCalced)
		{
			...
			auto vel = midas_playerio.GetPlayerVelocity().Length2D();
			loss = maxVel - vel;
...
{
	auto vel = midas_playerio.GetPlayerVelocity().Length2D();
	auto ducked = midas_playerio.m_fFlags.GetValue() & FL_DUCKING;
