...
ConVar y_midas_on_slide_pause_for("y_midas_on_slide_pause_for",
                                "0",
                                0,
...
void DMoMM::PreHook()
{
	...
	{
		InitConcommandBase(y_midas_on_slide_pause_for);
...
{
	if (midas_dmomm.sliding)
	{
		midas_dmomm.sliding = false;
		midas_dmomm.wasSliding = true;
	}
	else
	{
		midas_dmomm.wasSliding = false;
	}

	return midas_dmomm.ORIG_SlidingAndOtherStuff(thisptr, edx, a, b);
}
...
void DMoMM::HOOKED_MiddleOfSlidingFunction_Func()
{
	...
	if (!wasSliding)
	{
		const auto pauseFor = y_midas_on_slide_pause_for.GetInt();
		if (pauseFor > 0)
		{
...
__declspec(naked) void DMoMM::HOOKED_MiddleOfSlidingFunction()
{
	...
	midas_dmomm.HOOKED_MiddleOfSlidingFunction_Func();
	...
	__asm {
		...
		/**
			...
			jmp midas_dmomm.ORIG_MiddleOfSlidingFunction;
	}
}
#endif