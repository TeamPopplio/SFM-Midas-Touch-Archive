...
ConVar y_midas_hud_left("y_midas_hud_left", "0", FCVAR_CHEAT, "When set to 1, displays Midas HUD on the left.\n");

HUDFeature midas_hud;
...
void HUDFeature::LoadFeature()
{
	...
	{
		InitConcommandBase(y_midas_hud_left);
...
void HUDFeature::DrawHUD()
{
	...
	try
	{
		...
		{
			if (midas_overlay.renderingOverlay && callback.renderTime == RenderTime::overlay
			    && callback.shouldDraw())
			{
				callback.draw();
			}
			else if (!midas_overlay.renderingOverlay && callback.renderTime == RenderTime::paint
			         && callback.shouldDraw())
			{
				callback.draw();
			}
...
{
#ifndef OE
	if (midas_hud.loadingSuccessful && (mode == 2 || midas_overlay.renderingOverlay))
	{
		midas_hud.screen = (vrect_t*)midas_overlay.screenRect;
		midas_hud.DrawHUD();
	}

#endif

	midas_hud.ORIG_VGui_Paint(thisptr, edx, mode);
}
