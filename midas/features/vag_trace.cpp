...
ConVar y_midas_vag_trace("y_midas_vag_trace", "0", FCVAR_CHEAT, "Draws VAG trace.\n");
ConVar y_midas_vag_target("y_midas_vag_target", "0", FCVAR_CHEAT, "Draws VAG target trace.\n");
ConVar y_midas_vag_trace_portal(
    "y_midas_vag_trace_portal",
    "overlay",
    FCVAR_CHEAT,
...
VagTrace midas_vag_trace;

CON_COMMAND(y_midas_vag_target_set, "Set VAG target\n")
{
	midas_vag_trace.SetTarget(midas_playerio.GetPlayerEyePos());
}

void VagTrace::DrawTrace()
{
	bool vag_trace = y_midas_vag_trace.GetBool();
	bool vag_target = y_midas_vag_target.GetBool();
	if (!vag_trace && !vag_target)
		return;

	float lifeTime = midas_tickrate.GetTickrate() * 2;
	if (vag_target)
	{
	...
	if (strcmp(y_midas_vag_trace_portal.GetString(), "overlay") == 0)
	{
		enter_portal = getPortal(_y_midas_overlay_portal.GetString(), false);
	}
	else
	{
		enter_portal = getPortal(y_midas_vag_trace_portal.GetString(), false);
	}
...
void VagTrace::LoadFeature()
{
	if (interfaces::debugOverlay && TickSignal.Works)
	{
		...
		InitCommand(y_midas_vag_target_set);
		InitConcommandBase(y_midas_vag_trace);
		InitConcommandBase(y_midas_vag_target);
		InitConcommandBase(y_midas_vag_trace_portal);
	}
}
