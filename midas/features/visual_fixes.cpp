...
ConVar y_midas_motion_blur_fix("y_midas_motion_blur_fix", "0", FCVAR_ARCHIVE, "Fixes motion blur for startmovie.");
ConVar y_midas_disable_fade("y_midas_disable_fade", "0", FCVAR_ARCHIVE, "Disables all fades.");
ConVar y_midas_disable_shake("y_midas_disable_shake", "0", FCVAR_ARCHIVE, "Disables all shakes.");
ConVar y_midas_disable_tone_map_reset(
    "y_midas_disable_tone_map_reset",
    "0",
    FCVAR_ARCHIVE,
...
static VisualFixes midas_visual_fixes;
...
void VisualFixes::LoadFeature()
{
	if (ORIG_DoImageSpaceMotionBlur)
	{
		...
		InitConcommandBase(y_midas_motion_blur_fix);
	}

	if (ORIG_CViewEffects__Fade)
		InitConcommandBase(y_midas_disable_fade);

	if (ORIG_CViewEffects__Shake)
		InitConcommandBase(y_midas_disable_shake);

	if (ORIG_ResetToneMapping)
		InitConcommandBase(y_midas_disable_tone_map_reset);
}
...
void __cdecl VisualFixes::HOOKED_DoImageSpaceMotionBlur(void* view, int x, int y, int w, int h)
{
	...
	*/
	if (midas_visual_fixes.pgpGlobals)
	{
		if (y_midas_motion_blur_fix.GetBool())
		{
			origgpGlobals = *midas_visual_fixes.pgpGlobals;
			*midas_visual_fixes.pgpGlobals = *midas_visual_fixes.pgpGlobals + 12;
		}
	}

	midas_visual_fixes.ORIG_DoImageSpaceMotionBlur(view, x, y, w, h);

	if (midas_visual_fixes.pgpGlobals)
	{
		if (y_midas_motion_blur_fix.GetBool())
		{
			*midas_visual_fixes.pgpGlobals = origgpGlobals;
		}
	}
}
...
{
	if (!y_midas_disable_fade.GetBool())
		midas_visual_fixes.ORIG_CViewEffects__Fade(thisptr, edx, data);
}

void __fastcall VisualFixes::HOOKED_CViewEffects__Shake(void* thisptr, int edx, void* data)
{
	if (!y_midas_disable_shake.GetBool())
		midas_visual_fixes.ORIG_CViewEffects__Shake(thisptr, edx, data);
}

void __cdecl VisualFixes::HOOKED_ResetToneMapping(float value)
{
	if (!y_midas_disable_tone_map_reset.GetBool())
		midas_visual_fixes.ORIG_ResetToneMapping(value);
}
