...
ConVar y_midas_draw_portal_env("y_midas_draw_portal_env",
                             "0",
                             FCVAR_CHEAT | FCVAR_DONTRECORD,
                             "Draw the geometry in a portal's physics environment");

ConVar y_midas_draw_portal_env_type(
    "y_midas_draw_portal_env_type",
    "auto",
    FCVAR_CHEAT | FCVAR_DONTRECORD,
    "Usage: y_midas_draw_portal_env_type collide|auto|blue|orange|<index>; draw world collision and static props in a portal environment\n"
...
ConVar y_midas_draw_portal_env_ents("y_midas_draw_portal_env_ents",
                                  "0",
                                  FCVAR_CHEAT | FCVAR_DONTRECORD,
                                  "Draw entities owned by portal and shadow clones from remote portal");

ConVar y_midas_draw_portal_env_remote("y_midas_draw_portal_env_remote",
                                    "0",
                                    FCVAR_CHEAT | FCVAR_DONTRECORD,
                                    "Draw geometry from remote portal");

ConVar y_midas_draw_portal_env_wireframe("y_midas_draw_portal_env_wireframe",
                                       "1",
                                       FCVAR_CHEAT | FCVAR_DONTRECORD,
...
{
	const Vector camPos = midas_generic.GetCameraOrigin();
...
{
	if (!y_midas_draw_portal_env.GetBool() || !interfaces::engine_server->PEntityOfEntIndex(0))
		return nullptr;

	const char* typeStr = y_midas_draw_portal_env_type.GetString();
...
void DrawSgCollision()
{
	...
	WireframeMode mode = y_midas_draw_portal_env_wireframe.GetBool() ? FacesAndWire : OnlyFaces;
	...
	if (y_midas_draw_portal_env_remote.GetBool())
	{
	...
	if (y_midas_draw_portal_env_ents.GetBool())
	{
...
protected:
	...
	{
		return midas_physvis.ShouldLoadFeature() && midas_generic.ShouldLoadFeature()
	...
	{
		if (midas_physvis.AddPhysVisCallback({DrawSgCollision, "f"}))
		{
			InitConcommandBase(y_midas_draw_portal_env);
			InitConcommandBase(y_midas_draw_portal_env_type);
			InitConcommandBase(y_midas_draw_portal_env_ents);
			InitConcommandBase(y_midas_draw_portal_env_remote);
			InitConcommandBase(y_midas_draw_portal_env_wireframe);
		}
	};
...
static SgCollideVisFeature midas_sgCollision;
