...
{
	if (midas_tracing.overrideMinMax)
	{
		return midas_tracing._maxs;
	}
	else
		return midas_tracing.ORIG_CGameMovement__GetPlayerMaxs(thisptr, edx);
}
...
const Vector& __fastcall Tracing::HOOKED_CGameMovement__GetPlayerMins(void* thisptr, int edx)
{
	if (midas_tracing.overrideMinMax)
	{
		return midas_tracing._mins;
	}
	else
		return midas_tracing.ORIG_CGameMovement__GetPlayerMins(thisptr, edx);
}
...
double trace_fire_portal(QAngle angles, Vector& normal)
{
	...
	trace_t tr;
	midas_tracing.TraceFirePortal(tr, midas_generic.GetCameraOrigin(), fwd);
...
CON_COMMAND(
    y_midas_find_seam_shot,
    "y_midas_find_seam_shot [<pitch1> <yaw1> <pitch2> <yaw2> <epsilon>] - tries to find a seam shot on a \"line\" between viewangles (pitch1; yaw1) and (pitch2; yaw2) with binary search. Decreasing epsilon will result in more viewangles checked. A default value is 0.00001. If no arguments are given, first invocation selects the first point, second invocation selects the second point and searches between them.")
{
	QAngle a, b;
	...
	else
	{
		...
		{
			Msg("Usage: y_midas_find_seam_shot <pitch1> <yaw1> <pitch2> <yaw2> <epsilon> - tries to find a seam shot on a \"line\" between viewangles (pitch1; yaw1) and (pitch2; yaw2) with binary search. Decreasing epsilon will result in more viewangles checked. A default value is 0.00001. If no arguments are given, first invocation selects the first point, second invocation selects the second point and searches between them.\n");
			return;
		}
	...
	if (!midas_tracing.ORIG_GetActiveWeapon(utils::GetServerPlayer()))
	{
...
void Tracing::LoadFeature()
{
	...
	{
		InitCommand(
		    y_midas_find_seam_shot); // 5135 only but version detection doesnt properly exist, so cba to fix this
	}
#endif
	...
	if (ORIG_CEngineTrace__PointOutsideWorld)
	{
		AddHudCallback(
		    "oob",
		    []() {
			    Vector v = midas_generic.GetCameraOrigin();
			    trace_t tr;
			    Strafe::Trace(tr, v, v + Vector(1, 1, 1));

			    int oob = midas_tracing.ORIG_CEngineTrace__PointOutsideWorld(nullptr, 0, v) && !tr.startsolid;
			    midas_hud.DrawTopHudElement(L"oob: %d", oob);
		    },
		    y_midas_hud_oob);
	}
#endif
