...
ConVar y_midas_prevent_vag_crash(
    "y_midas_prevent_vag_crash",
    "0",
...
void VAG::LoadFeature()
{
	...
	{
		InitConcommandBase(y_midas_prevent_vag_crash);
...
__declspec(naked) void VAG::HOOKED_MiddleOfTeleportTouchingEntity()
{
	...
	*/
	__asm {
		...
		popfd;
		popad;
		jmp midas_vag.ORIG_MiddleOfTeleportTouchingEntity;
	}
}
...
__declspec(naked) void VAG::HOOKED_EndOfTeleportTouchingEntity()
{
	...
	midas_vag.HOOKED_EndOfTeleportTouchingEntity_Func();
	__asm {
		popfd;
		popad;
		jmp midas_vag.ORIG_EndOfTeleportTouchingEntity;
	}
}
...
{
	if (!midas_vag.ORIG_EndOfTeleportTouchingEntity || !y_midas_prevent_vag_crash.GetBool())
		return;
	if (midas_vag.recursiveTeleportCount++ > 2)
	{
		Msg("midas:  nudging entity to prevent more recursive teleports!\n");
		Vector* entPos = (Vector*)((uint32_t*)tpStackPointer + 26);
		Vector* portalNorm = *((Vector**)portalPtr + 2505) + 2;
		DevMsg(
		    "midas:  ent coords in TeleportTouchingEntity: %f %f %f, portal norm: %f %f %f, %i recursive teleports\n",
		    entPos->x,
		    entPos->y,
		    entPos->z,
		    portalNorm->x,
		    portalNorm->y,
		    portalNorm->z,
		    midas_vag.recursiveTeleportCount);
...
{
	if (y_midas_prevent_vag_crash.GetBool())
