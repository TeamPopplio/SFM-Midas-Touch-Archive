...
VagSearcher midas_vag_searcher;

ConVar y_midas_vag_search_portal(
    "y_midas_vag_search_portal",
    "overlay",
    FCVAR_CHEAT,
    "Chooses the portal for the VAG search. Valid options are overlay/blue/orange/portal index. This is the portal you enter.\n");

CON_COMMAND(y_midas_vag_search, "Search VAG")
{
	midas_vag_searcher.StartSearch();
}
...
void VagSearcher::StartSearch()
{
	...
	if (strcmp(y_midas_vag_search_portal.GetString(), "overlay") == 0)
	{
		enter_portal = getPortal(_y_midas_overlay_portal.GetString(), false);
	}
	else
	{
		enter_portal = getPortal(y_midas_vag_search_portal.GetString(), false);
	}
...
{
	InitCommand(y_midas_vag_search);
	InitConcommandBase(y_midas_vag_search_portal);
	if (TickSignal.Works)
	{
...
void VagSearcher::StartIterations()
{
	...
	{
		Msg("Auto enable y_midas_prevent_vag_crash.\n");
		y_midas_prevent_vag_crash.SetValue(1);
	}
