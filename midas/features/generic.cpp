...
{
	FrameSignal();
	midas_generic.ORIG_HudUpdate(bActive);
}

bool __cdecl GenericFeature::HOOKED_SV_ActivateServer()
{
	bool result = midas_generic.ORIG_SV_ActivateServer();
...
{
	midas_generic.ORIG_FinishRestore(thisptr, edx);
...
void __fastcall GenericFeature::HOOKED_SetPaused(void* thisptr, int edx, bool paused)
{
	...
	if (paused == false)
	{
		if (midas_generic.shouldPreventNextUnpause)
		{
			DevMsg("Unpause prevented.\n");
			midas_generic.shouldPreventNextUnpause = false;
			return;
		}
	}

	return midas_generic.ORIG_SetPaused(thisptr, edx, paused);
}

void __fastcall GenericFeature::HOOKED_AdjustAngles(void* thisptr, int edx, float frametime)
{
	midas_playerio.Set_cinput_thisptr(thisptr);
	midas_generic.ORIG_AdjustAngles(thisptr, edx, frametime);

	float va[3];
	bool yawChanged = false;

	if (!midas_playerio.pCmd)
	{
		return;
	}

	EngineGetViewAngles(va);
	midas_aim.HandleAiming(va, yawChanged);

	if (midas_tas.tasAddressesWereFound && tas_strafe.GetBool())
	{
		midas_tas.Strafe(va, yawChanged);
	}
	...
	{
		OngroundSignal(midas_playerio.IsGroundEntitySet());
		AdjustAngles();
	}
}
