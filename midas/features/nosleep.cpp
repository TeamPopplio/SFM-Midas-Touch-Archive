...
ConVar y_midas_focus_nosleep("y_midas_focus_nosleep", "0", 0, "Improves FPS while alt-tabbed.");
...
static NoSleepFeature midas_nosleep;
...
void NoSleepFeature::LoadFeature()
{
	...
	{
		InitConcommandBase(y_midas_focus_nosleep);
	}
}
...
{
	if (y_midas_focus_nosleep.GetBool())
		nMaxSleepTimeMS = 0;
	midas_nosleep.ORIG_CInputSystem__SleepUntilInput(thisptr, edx, nMaxSleepTimeMS);
}
