...
static CvarStuff midas_cvarstuff;

CON_COMMAND(y_midas_cvar, "CVar manipulation.")
{
	if (!g_pCVar)
		return;
	...
	if (args.ArgC() < 2)
	{
		Msg("Usage: y_midas_cvar <name> [value]\n");
		return;
	}
...
CON_COMMAND(y_midas_cvar_random, "Randomize CVar value.")
{
	if (!g_pCVar)
		return;
	...
	if (args.ArgC() != 4)
	{
		Msg("Usage: y_midas_cvar_random <name> <min> <max>\n");
		return;
	}
...
#if !defined(OE)
CON_COMMAND(y_midas_cvar2, "CVar manipulation, sets the CVar value to the rest of the argument string.")
{
	if (!g_pCVar)
		return;
...
void CvarStuff::LoadFeature()
{
	...
	{
		InitCommand(y_midas_cvar);
		InitCommand(y_midas_cvar_random);
	}
#else
