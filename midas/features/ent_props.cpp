...
CON_COMMAND(y_midas_canjb, "Tests if player can jumpbug on a given height, with the current position and speed.")
{
	if (args.ArgC() < 2)
	{
		Msg("Usage: y_midas_canjb [height]\n");
		return;
	}
...
CON_COMMAND(y_midas_print_portals, "Prints all portal indexes, their position and angles.")
{
