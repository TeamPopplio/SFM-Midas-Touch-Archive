...
ConVar y_midas_hud_script_length("y_midas_hud_script_progress", "0", FCVAR_CHEAT, "Turns on the script progress hud.\n");

#ifdef OE
ConVar y_midas_gamedir(
    "y_midas_gamedir",
    "",
    0,
...
void TASFeature::Strafe(float* va, bool yawChanged)
{
	...
	auto pl = midas_playerio.GetPlayerData();
	...
	midas_playerio.SetTASInput(va, out);
}
...
void TASFeature::LoadFeature()
{
	if (AfterFramesSignal.Works)
	{
...
#ifdef OE
		InitConcommandBase(y_midas_gamedir);
#endif
		...
		AddHudCallback(
		    "frame",
		    [this]() {
			    midas_hud.DrawTopHudElement(L"frame: %d / %d",
			                              scripts::g_TASReader.GetCurrentTick(),
			                              scripts::g_TASReader.GetCurrentScriptLength());
		    },
		    y_midas_hud_script_length);
#endif
	}

	tasAddressesWereFound = midas_generic.ORIG_AdjustAngles && midas_playerio.PlayerIOAddressesFound();
	...
	if (tasAddressesWereFound)
	{
		...
		// Tracing related
		if (midas_tracing.CanTracePlayerBBox())
		{
