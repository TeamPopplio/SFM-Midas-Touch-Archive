...
namespace scripts
{
	...
	{
		return GenerateVectorData(midas_playerio.GetPlayerVelocity(), decimals);
	}

	ValidationResult VelocityTracker::Validate(const std::string& expectedValue) const
	{
		return VectorValidation(midas_playerio.GetPlayerVelocity(), expectedValue, decimals);
	}
	...
	{
		return GenerateVectorData(midas_playerio.GetPlayerEyePos(), decimals);
	}

	ValidationResult PosTracker::Validate(const std::string& expectedValue) const
	{
		return VectorValidation(midas_playerio.GetPlayerEyePos(), expectedValue, decimals);
	}
