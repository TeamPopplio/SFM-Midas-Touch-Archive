...
namespace scripts
{
	...
	void SourceTASReader::ExecuteScriptWithResume(const std::string& script, int resumeTicks)
	{
		...
		midas_afterframes.AddAfterFramesEntry(afterframes_entry_t(0, "y_midas_cvar fps_max 0; mat_norendering 1"));
		tickTime = midas_tickrate.GetTickrate();
		snprintf(buffer, ARRAYSIZE(buffer), "y_midas_cvar fps_max %.6f; mat_norendering 0", 1 / tickTime);
		int resumeTick = GetCurrentScriptLength() - resumeTicks;
		midas_afterframes.AddAfterFramesEntry(afterframes_entry_t(resumeTick, buffer));
	}
	...
	void SourceTASReader::CommonExecuteScript(bool search)
	{
		try
		{
			Reset();
#if OE
			const char* dir = y_midas_gamedir.GetString();
			if (dir == NULL || dir[0] == '\0')
				Msg("WARNING: Trying to load a script file without setting the game directory with y_midas_gamedir in old engine!\n");
#endif
	...
	void SourceTASReader::Execute()
	{
		...
		SetFpsAndPlayspeed();
		midas_demostuff.Demo_StopRecording();
		currentTick = 0;
