...
namespace scripts
{
	...
	void Field1(FrameBulkInfo& frameBulkInfo)
	{
		...
		if (frameBulkInfo.ContainsFlag(AUTOJUMP, "j"))
			frameBulkInfo.AddCommand("y_midas_autojump 1");
		else
			frameBulkInfo.AddCommand("y_midas_autojump 0");

		frameBulkInfo.AddPlusMinusCmd("y_midas_duckspam", frameBulkInfo.ContainsFlag(DUCKSPAM, "d"));
	...
	void Field4_5(FrameBulkInfo& frameBulkInfo)
	{
		if (frameBulkInfo.IsFloat(YAW_KEY))
		{
			if (frameBulkInfo.ContainsFlag(STRAFE, "s"))
				...
				frameBulkInfo.AddCommand("_y_midas_setyaw " + frameBulkInfo[YAW_KEY]);
		}
		...
		if (frameBulkInfo.IsFloat(PITCH_KEY))
			frameBulkInfo.AddCommand("_y_midas_setpitch " + frameBulkInfo[PITCH_KEY]);
