...
namespace scripts
{
	...
	void ParsedScript::AddSaveLoad()
	{
		...
		AddAfterFramesEntry(afterFramesTick,
		                    "save " + sName + "; load " + sName + +";  _y_midas_afterframes_await_load");
		// Only manually record if autorecording isn't available
		if (!midas_demostuff.Demo_IsAutoRecordingAvailable() && !demoName.empty())
	...
	void ParsedScript::Reset()
	{
		...
		initCommand =
		    "sv_cheats 1; y_midas_pause 0;_y_midas_afterframes_await_load; _y_midas_afterframes_reset_on_server_activate 0; _y_midas_resetpitchyaw;"
		    "tas_aim_reset";
		duringLoad.clear();
		saveName.clear();
		saveStates.clear();
