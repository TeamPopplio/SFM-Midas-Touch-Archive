...
namespace Strafe
{
	bool CanTrace()
	{
		...
		{
			return midas_tracing.ORIG_UTIL_TraceRay != nullptr;
		}
		else
		{
			return midas_tracing.CanTracePlayerBBox();
		}
	}
	...
	void TracePlayer(trace_t& trace, const Vector& start, const Vector& end, HullType hull)
	{
		...
		if (tas_strafe_version.GetInt() == 1)
		{
			...
			midas_tracing.ORIG_UTIL_TraceRay(ray,
		...
		else
		{
			...
			SetMoveData();
			midas_tracing.TracePlayerBBox(start,
			                            end,
			                            mins,
			                            maxs,
			                            MASK_PLAYERSOLID,
	...
	{
#ifndef OE
		if (!midas_tracing.ORIG_UTIL_TraceRay)
			return;
	...
	PositionType GetPositionType(PlayerData& player, HullType hull)
	{
		...
		{
			if (midas_playerio.IsGroundEntitySet())
		...
		else
		{
			...
			midas_tracing.TracePlayerBBox(bumpOrigin,
			                            point,
			                            mins,
			                            maxs,
			                            MASK_PLAYERSOLID,
			...
			if (utils::DoesGameLookLikePortal())
				midas_tracing.ORIG_TracePlayerBBoxForGround2(bumpOrigin,
				                                           point,
				                                           mins,
				                                           maxs,
				...
				midas_tracing.ORIG_TracePlayerBBoxForGround(bumpOrigin,
				                                          point,
				                                          mins,
				                                          maxs,
				                                          utils::GetServerPlayer(),
